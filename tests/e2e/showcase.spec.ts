...
const openShowcase = async (page: Page, path = "/actions/"): Promise<void> => {
...
test("opens on the brand and navigates between focused pages", async ({
  page,
}) => {
  await openShowcase(page, "/");

  await expect(
    page.getByRole("heading", { level: 1, name: "Astilba" })
  ).toBeVisible();
  await expect(page.getByRole("link", { name: "Brand" })).toHaveAttribute(
    "aria-current",
    "page"
  );

  await page.getByRole("link", { name: "Actions" }).click();
  await expect(page).toHaveURL(/\/actions\/$/u);
  await expect(
...
test("selects, copies, and exports official brand assets", async ({ page }) => {
  await openShowcase(page, "/");

  const variants = page.locator(".brand-variants > button");
  await expect(variants).toHaveCount(5);

  await page.getByRole("button", { name: "Wordmark" }).click();
  await expect(
    page.getByRole("heading", { level: 2, name: "Wordmark" })
  ).toBeVisible();
  await expect(page.getByText("Selected asset")).toHaveCount(0);
  await expect(page.getByAltText("Wordmark preview")).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Download SVG" })
  ).toHaveAttribute("download", "astilba-wordmark-light.svg");

  const svgTab = page.getByRole("tab", { name: "SVG" });
  const svgPanel = page.getByRole("tabpanel", { name: "SVG" });
  const tabsIndicator = page.locator(".astilba-tabs-indicator");
  const svgCodeArea = svgPanel.locator(".brand-code-scroll-area");
  const codeToolbar = page.locator(".brand-code-toolbar");
  const codeCopyButton = codeToolbar.getByRole("button", {
    name: "Copy code",
  });
  await expect(tabsIndicator).toBeVisible();
  await expect(codeCopyButton).toBeVisible();
  await expect(
    page
      .getByRole("tablist", { name: "Asset code formats" })
      .getByRole("button", { name: "Copy code" })
  ).toHaveCount(0);
  await expect(codeCopyButton).toHaveCSS("height", "32px");
  await expect(codeCopyButton).toHaveCSS("width", "32px");
  await codeCopyButton.hover();
  await expect(page.getByRole("tooltip", { name: "Copy code" })).toBeVisible();
  await expect(tabsIndicator).toHaveCSS("transition-duration", "0.25s");
  await expect(svgCodeArea).toHaveCSS("height", "144px");
  await expect(svgPanel).not.toContainText("Generated by Pixelmator");
  await expect(svgPanel).not.toContainText("<?xml");
  await svgTab.focus();
  await page.keyboard.press("ArrowRight");
  const jsxTab = page.getByRole("tab", { name: "JSX" });
  await expect(jsxTab).toBeFocused();
  await expect(page.getByRole("tabpanel", { name: "JSX" })).toContainText(
    "export function AstilbaWordmarkLight"
  );
  await expect(page.getByRole("tabpanel", { name: "JSX" })).toContainText(
    "fillRule="
  );
  await expect(page.getByRole("tabpanel", { name: "JSX" })).not.toContainText(
    "fill-rule="
  );
  await expect(
    page
      .getByRole("tabpanel", { name: "JSX" })
      .locator(".brand-code-scroll-area")
  ).toHaveCSS("height", "144px");
  await expect
    .poll(async () => {
      const [indicatorBounds, tabBounds] = await Promise.all([
        tabsIndicator.boundingBox(),
        jsxTab.boundingBox(),
      ]);
      if (!(indicatorBounds && tabBounds)) {
        return Number.POSITIVE_INFINITY;
      }
      return Math.max(
        Math.abs(indicatorBounds.x - tabBounds.x),
        Math.abs(indicatorBounds.width - tabBounds.width)
      );
    })
  ...
  expect(downloadedPng.suggestedFilename()).toBe(
    "astilba-wordmark-light-32.png"
  );

  const downloadTrigger = page.getByRole("button", {
    name: "More download formats",
  });
  await downloadTrigger.click();
  const triggerBounds = await downloadTrigger.boundingBox();
...
test("reports brand copy and export failures without unhandled promises", async ({
  page,
}) => {
  await page.addInitScript(() => {
    ...
    HTMLImageElement.prototype.decode = () =>
      Promise.reject(new Error("Image decoding unavailable"));
  });
  await openShowcase(page, "/");

  await page.getByRole("button", { name: "Copy code" }).click();
  await expect(page.getByText("Code could not be copied.")).toBeVisible();
  await expect(page.getByRole("tooltip", { name: "Copied!" })).toHaveCount(0);

  await page.getByRole("button", { name: "32px" }).click();
  await expect(page.getByText("PNG could not be exported.")).toBeVisible();
});
...
test("renders the control contract in both themes", async ({ page }) => {
  ...
  await page.locator(".showcase-nav").evaluate((element) => {
    element.hidden = true;
  });
  const specimens = page.locator(".specimen-section:not([hidden])");
...
test("preserves collapsible semantics, focus, and motion", async ({ page }) => {
  await openShowcase(page, "/disclosures/");
...
test("preserves field guidance, validation, focus, and themes", async ({
  ...
  await openShowcase(page, "/fields/");
...
test("explains vertical overflow and reveals its scrollbar on interaction", async ({
  ...
  await openShowcase(page, "/scroll-areas/");
...
test("removes tooltip motion when reduced motion is requested", async ({
  ...
  await openShowcase(page, "/");
  await expect(page.locator(".astilba-tabs-indicator")).toHaveCSS(
    "transition-duration",
    "0s"
  );
});
...
test("has no detectable accessibility violations in either theme", async ({
  ...
  await openShowcase(page, "/actions/");
  await page.getByRole("button", { name: "Use light theme" }).first().click();
  await expect(page.locator("html")).toHaveAttribute("data-theme", "light");
...
test("keeps the showcase within a narrow viewport", async ({ page }) => {
  ...
  await openShowcase(page, "/");
  const brandDimensions = await page.evaluate(() => ({
    clientWidth: document.documentElement.clientWidth,
    scrollWidth: document.documentElement.scrollWidth,
  }));
  expect(brandDimensions.scrollWidth).toBe(brandDimensions.clientWidth);
